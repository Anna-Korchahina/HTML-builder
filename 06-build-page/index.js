...
fs.writeFile(htmlDirection, '', (err) => {
  if (err) {
    throw err;
  }
});

fs.writeFile(csslDirection, '', (err) => {
  if (err) {
    throw err;
  }
});
...
async function copyDirectory() {
  try {
    ...
    for (let i = 0; i < readDir.length; i++) {
      let copies = await fsPromise.readdir(
        `06-build-page/assets/${readDir[i].name}`,
        { withFileTypes: true },
      );
      console.log(readDir[i].name);
      fs.mkdir(
        `06-build-page/project-dist/assets/${readDir[i].name}`,
        { recursive: true },
        (err) => {
          if (err) {
            throw err;
      ...
      let readyCopy = await fsPromise.readdir(
        `06-build-page/project-dist/assets/${readDir[i].name}`,
        { withFileTypes: true },
      );
      for (const file of readyCopy) {
        fs.unlink(
          `06-build-page/project-dist/assets/${readDir[i].name}/${file.name}`,
          function (err) {
            if (err) {
              console.log(err);
            }
      ...
      for (const copy of copies) {
        ...
        fs.copyFile(
          `06-build-page/assets/${readDir[i].name}/${copy.name}`,
          `06-build-page/project-dist/assets/${readDir[i].name}/${copy.name}`,
          (err) => {
            if (err) {
...
async function createHtml() {
  try {
    let templateHtml = await fsPromise.readFile(
      __dirname + '/' + 'template.html',
    );
    let htmlComponents = await fsPromise.readdir(
    ...
    for (const component of htmlComponents) {
      if (component.isFile() && path.extname(component.name) === '.html') {
        currentPart = await fsPromise.readFile(
...
async function copyStyles() {
  try {
    let stylesComponents = await fsPromise.readdir(__dirname + '/' + 'styles', {
      withFileTypes: true,
    });
    for (const styles of stylesComponents) {
      if (styles.isFile() === true && path.extname(styles.name) == '.css') {
        let currentCss = await fsPromise.readFile(
          __dirname + `/styles/${styles.name}`,
          'utf-8',
        );
        fs.appendFile(
